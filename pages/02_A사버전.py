...
# ... (이전과 동일한 st.set_page_config, markdown, st.title, st.info, angle_options, default_funcs_info 등) ...
...
if st.session_state.selected_func_type == "사용자 정의 함수 입력":
    ...
    if not func_input_str.strip(): # 입력이 비어있으면 기본 함수로 대체
            ...
            for p_start in potential_starts:
                ...
                try:
                    ...
                    if best_res is None or (res_temp.success and np.isfinite(res_temp.fun) and res_temp.fun < best_res.fun) or \
                       (res_temp.success and np.isfinite(res_temp.fun) and not best_res.success): # 유효한 fun 값만 비교
                        best_res = res_temp
                except Exception:
                    pass
...
def plot_graphs(f_np_func, dx_np_func, dy_np_func, x_min_curr, x_max_curr, y_min_curr, y_max_curr,
    ...
    # 극단값 제한 범위
    CLIP_MIN, CLIP_MAX = -1e4, 1e4 # 범위를 줄여서 Plotly의 안정성 확보 시도
    ...
    if gd_path_curr and len(gd_path_curr) > 0 and callable(f_np_func):
        ...
        px = np.array(px, dtype=float) # numpy 배열로 변환하여 클리핑 용이
        py = np.array(py, dtype=float)
        px = np.clip(px, x_min_curr, x_max_curr) # 경로도 그래프 범위 내로 클리핑
        py = np.clip(py, y_min_curr, y_max_curr)

        try:
            pz_raw = [f_np_func(pt_x, pt_y) for pt_x, pt_y in zip(px,py)] # 클리핑된 px, py 사용
            for val in pz_raw:
            ...
            pz = [0.0] * len(px) # 오류 시 0으로 채움

        # px, py, pz가 모두 유효한 리스트/배열인지, 길이가 동일한지 확인
        if not (isinstance(px, (list, np.ndarray)) and isinstance(py, (list, np.ndarray)) and isinstance(pz, (list, np.ndarray)) and \
            ...
            # 데이터가 유효하지 않으면 빈 경로를 그리거나, 오류 메시지 후 스킵
            pass # 또는 st.warning("경로 데이터가 유효하지 않아 그릴 수 없습니다.")
        else:
        ...
        if len(gd_path_curr) > 0 and not st.session_state.get("play", False) and callable(dx_np_func) and callable(dy_np_func) and pz:
            last_x_gd, last_y_gd = px[-1], py[-1] # 클리핑된 값 사용
            last_z_gd = pz[-1]
    ...
    if np.sum(np.isfinite(Zs_plot)) > 0 :
        ...
        if pz and any(np.isfinite(pz_val) for pz_val in pz): # pz도 클리핑됨
        ...
        # std도 클리핑된 값 기준이므로 너무 크지 않게
        plot_std = np.clip(plot_std, 0.1, (CLIP_MAX-CLIP_MIN)/10) 
        ...
        if z_min_val_for_layout >= z_max_val_for_layout: # 혹시 역전되면 강제 조정
    ...
    if not current_step_info_func:
        ...
        if st.session_state.gd_step < steps and next_x_info != 'N/A': 
             ...
             if all(isinstance(val, (int, float)) for val in [curr_x_info, learning_rate, grad_x_info, next_x_info, curr_y_info, grad_y_info, next_y_info if next_y_info != 'N/A' else 0.0]): # next_y_info가 N/A가 아닐때만 체크
...
# ... (이하 코드는 이전과 거의 동일, parse_error 처리 및 함수 호출 부분만 확인) ...
...
if parse_error and not (callable(f_np) and callable(dx_np) and callable(dy_np)):
    ...
    x_s_dummy, y_s_dummy = symbols('x y')
    f_sym_dummy = x_s_dummy**2 + y_s_dummy**2
    f_np_dummy = lambdify((x_s_dummy, y_s_dummy), f_sym_dummy, 'numpy')
    dx_f_sym_dummy = diff(f_sym_dummy, x_s_dummy); dy_f_sym_dummy = diff(f_sym_dummy, y_s_dummy)
    dx_np_dummy = lambdify((x_s_dummy, y_s_dummy), dx_f_sym_dummy, 'numpy'); dy_np_dummy = lambdify((x_s_dummy, y_s_dummy), dy_f_sym_dummy, 'numpy')
    
    gd_path_for_dummy = st.session_state.get("gd_path", [(0.,0.)])
    if not gd_path_for_dummy: gd_path_for_dummy = [(0.,0.)] # 비어있을 경우 대비
    ...
    if 'graph_placeholder_3d' not in globals(): graph_placeholder_3d = st.empty() # 플레이스홀더가 없을 경우 생성
...
with col_btn1: step_btn = st.button("🚶 한 스텝 이동", use_container_width=True, disabled=st.session_state.get("play", False) or parse_error or not callable(f_np))
with col_btn2: play_btn = st.button("▶️ 전체 실행" if not st.session_state.get("play", False) else "⏹️ 중지", key="playbtn_widget_key", use_container_width=True, disabled=parse_error or not callable(f_np))
with col_btn3: reset_btn = st.button("🔄 초기화", key="resetbtn_widget_key", use_container_width=True, disabled=st.session_state.get("play", False) or parse_error or not callable(f_np))
...
def perform_one_step():
    ...
    if st.session_state.gd_step < steps:
        ...
        try:
            if not (callable(f_np) and callable(dx_np) and callable(dy_np)): # 함수 호출 가능성 확인
...
if reset_btn:
    ...
    apply_preset_for_func_type(st.session_state.selected_func_type)
    if st.session_state.selected_func_type == "사용자 정의 함수 입력":
        st.session_state.user_func_input = "x**2 + y**2" # 사용자 정의 함수도 리셋
    ...
    # Reset f_np, dx_np, dy_np to reflect the reset function
    try:
        ...
        parse_error = False # 에러 상태도 리셋
    except Exception:
        parse_error = True # 리셋 중에도 파싱 에러가 날 수 있음

    st.rerun()

if step_btn:
    if not st.session_state.get("play", False) and callable(f_np) and callable(dx_np) and callable(dy_np):
        st.session_state.play = False
        perform_one_step()
        st.rerun()
...
if callable(f_np) and callable(dx_np) and callable(dy_np):
    if st.session_state.get("play", False) and st.session_state.gd_step < steps:
        ...
        if perform_one_step():
            ...
            time.sleep(0.25)
            if st.session_state.gd_step < steps and st.session_state.play:
                st.rerun()
            else:
                st.session_state.play = False
            ...
            graph_placeholder_2d.plotly_chart(fig2d, use_container_width=True)
            step_info_placeholder.markdown(info_md, unsafe_allow_html=True)
            # st.rerun() # 여기서 rerun하면 메시지가 바로 안 보일 수 있음
    else:
...
displayed_errors = set() # 중복 에러 메시지 방지용
