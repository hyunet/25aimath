...
# 버튼 UI를 먼저 배치하여 그래프 위에 나타나도록 함
# 스트림릿 컨테이너 이용하여 순서 배치
button_container = st.container()

# 그래프를 표시할 컨테이너
graph_container = st.container()

# 에러 정보 표시를 위한 임시 플레이스홀더
temp_error_placeholder = st.empty()
...
if parse_error and not (callable(f_np) and callable(dx_np) and callable(dy_np)):
    temp_error_placeholder.warning("함수 오류로 인해 시뮬레이션을 진행할 수 없습니다. 사이드바에서 함수 정의를 수정해주세요.")
    x_s_dummy, y_s_dummy = symbols('x y')
    f_sym_dummy = x_s_dummy**2 + y_s_dummy**2
