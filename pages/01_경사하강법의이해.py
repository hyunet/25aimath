...
angle_options = {
    ...
    "정면(x+방향)": dict(x=2.0, y=0.0, z=0.5), # 기본 시점으로 사용할 예정
...
default_func_type = func_options[0] # 기본 함수: 볼록 함수
...
if "x_min_max_slider" not in st.session_state:
    st.session_state.x_min_max_slider = default_x_range
if "y_min_max_slider" not in st.session_state:
    st.session_state.y_min_max_slider = default_y_range
if "start_x_slider" not in st.session_state: 
    st.session_state.start_x_slider = default_start_x
if "start_y_slider" not in st.session_state: 
    st.session_state.start_y_slider = default_start_y
if "learning_rate_input" not in st.session_state: 
    st.session_state.learning_rate_input = default_lr
if "steps_slider" not in st.session_state: 
    st.session_state.steps_slider = default_steps

# --- 2. 현재 설정값 결정 (세션 상태 기반) ---
# 이 값들은 UI 위젯이 변경될 때마다 on_change 콜백을 통해 st.session_state가 업데이트되고,
# 스크립트 재실행 시 이 부분에서 최신 st.session_state 값을 읽어 다시 할당됨.
...
if st.session_state.selected_func_type == "사용자 정의 함수 입력":
    ...
    # default_funcs에 해당 키가 없을 경우를 대비 (거의 발생 안 함)
...
# 파라미터가 변경되면 경로를 초기화하기 위한 조건들
# (UI 위젯 값 자체가 아니라, 그것으로 계산된 설정값을 사용)
if "gd_path" not in st.session_state or \
   ...
   st.session_state.get("last_start_y_eval", 0.0) != start_y:

    st.session_state.gd_path = [(float(start_x), float(start_y))]
    st.session_state.gd_step = 0
    st.session_state.play = False 
    st.session_state.last_func_eval = func_input # 비교용으로 현재 실제 사용된 func_input 저장
...
with st.sidebar:
    ...
    with st.expander("💡 경사 하강법이란?", expanded=False):
        st.markdown("""
        경사 하강법은 함수의 최솟값을 찾기 위한 기본적인 1차 최적화 알고리즘입니다... (이전 내용과 동일)
        """)
    with st.expander("📖 주요 파라미터 가이드", expanded=False):
        st.markdown(f"""
        - **함수 $f(x, y)$ 선택**: 최적화하려는 대상 함수입니다... (이전 내용과 동일)
        """)
    ...
    # 시작 위치 슬라이더의 min/max는 현재 x_min, x_max (세션 상태에서 가져옴)를 사용
    current_x_min, current_x_max = st.session_state.x_min_max_slider
    current_y_min, current_y_max = st.session_state.y_min_max_slider
    st.slider("시작 x 위치", float(current_x_min), float(current_x_max), st.session_state.start_x_slider, step=0.1, 
              key="start_x_key_widget", 
              on_change=lambda: setattr(st.session_state, "start_x_slider", st.session_state.start_x_key_widget))
    st.slider("시작 y 위치", float(current_y_min), float(current_y_max), st.session_state.start_y_slider, step=0.1, 
    ...
    scipy_result_placeholder = st.sidebar.empty() # SciPy 결과를 표시할 위치
...
def plot_gd(f_np_func, dx_np_func, dy_np_func, x_min_curr, x_max_curr, y_min_curr, y_max_curr, gd_path_curr, min_point_scipy_curr, current_camera_eye_func):
    ...
    fig.add_trace(go.Scatter3d(
        ...
        marker=dict(size=10, color='yellow', symbol='circle', line=dict(color='black', width=1)), # 노란색 원, 검은 테두리
...
try:
    ...
    f_np_parsed = lambdify((x_sym, y_sym), f_sym_parsed, modules=['numpy', {'cos': np.cos, 'sin': np.sin, 'exp': np.exp, 'sqrt': np.sqrt, 'pi': np.pi}]) # pi 추가
    ...
    try: 
        ...
        best_res = None
        # Himmelblau 같은 함수는 여러 시작점에서 시도하는 것이 좋음
        if "Himmelblau" in st.session_state.selected_func_type:
            potential_starts.extend([[3,2], [-2.8, 3.1], [-3.7, -3.2], [3.5, -1.8]])
        ...
        if best_res and best_res.success:
            ...
            *경사 하강법의 목표는 이와 같은 최적점에 도달하는 것입니다.*
            ...
            scipy_result_placeholder.info("SciPy 최적점을 찾지 못했습니다.")

    except Exception as e_scipy: 
        scipy_result_placeholder.warning(f"SciPy 오류: {str(e_scipy)[:100]}...")
...
if reset_btn:
    ...
    st.session_state.messages = []
    st.session_state.last_func_eval = default_funcs[st.session_state.selected_func_type] # 초기화 시 last_func도 업데이트
...
if play_btn: 
    if not st.session_state.get("play", False): 
        ...
        # 애니메이션 시작 시 camera_eye는 상단에서 st.session_state.selected_camera_option_name 기반으로 계산된 값
        st.session_state.animation_camera_eye = camera_eye 
        st.session_state.messages = [] 
        st.rerun() 
